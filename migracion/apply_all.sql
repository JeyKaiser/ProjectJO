...
-- ╔══════════════════════════════════════════════════════════════
-- ║  PARTE 1: SCHEMA DDL (53 tablas)
-- ╚══════════════════════════════════════════════════════════════

-- ===========================================================================
-- AtelierData v2.0 â€” Schema PostgreSQL para GestiÃ³n de Colecciones JO
-- Corregido y completado desde informe_ejecutivo de anÃ¡lisis
-- ===========================================================================
-- Correcciones aplicadas:
--   âœ“ reference_codes estaba triplicada â†’ unificada en una sola definicion
--   âœ“ Orden FK corregido (referencias se crea antes que sus dependientes)
--   âœ“ Agregadas tablas criticas faltantes: consumos, contramuestras,
--     notas_fabricacion, molderia, laboratorios, entregables
--   âœ“ reference_fabrics limpia de campos duplicados del maestro fabrics
--   âœ“ Campos nuevos en references: drops, time_collection, maquila, liberaciones
-- ===========================================================================
...
-- ===========================================================================
-- 5. CODIGOS DE REFERENCIA (MD / PT) â€” UNICA definicion, corregida
-- ===========================================================================
...
--    referencia â†’ usage, width_cm y consumo_base son propios del contexto.
-- ===========================================================================
...
--    Trazabilidad completa: Creativo â†’ TÃ©cnico â†’ Trazador â†’ Contramuestra
...
-- ===========================================================================
-- 19. IMAGENES (tabla polimÃ³rfica unificada)
-- ===========================================================================
...
-- ╔══════════════════════════════════════════════════════════════
-- ║  PARTE 2: SEED CATALOGOS
-- ╚══════════════════════════════════════════════════════════════

-- ===========================================================================
-- AtelierData v2.0 â€” Seed de Datos Maestros (CatÃ¡logos)
...
('WS', 'WINTER SUN',       'WS', 2026, 'Coleccion Winter Sun â€” temporada invierno/verano'),
...
('READY TO WEAR',     'Linea principal de prÃªt-Ã -porter'),
('SWIMWEAR',          'Linea de trajes de baÃ±o'),
...
((SELECT id FROM line_ids WHERE name = 'SWIMWEAR'),      'ONE PIECE','Enterizos de baÃ±o'),
...
('CREATIVO',          'Disenador creativo â€” crea molderia base y muestra diseno'),
('TECNICO',           'Disenador tecnico â€” escala moldes y calcula consumos'),
('TRAZADOR',          'Trazador â€” experto en reduccion de consumos y trazos'),
('MODISTA',           'Modista â€” confecciona muestras y contramuestras'),
('SUPERVISOR',        'Supervisor de taller o produccion'),
('CONTROL_CALIDAD',   'Control de calidad â€” verifica prendas terminadas'),
...
('BAJA',   'Dificultad baja â€” procesos simples'),
('MEDIA',  'Dificultad media â€” procesos moderados'),
('ALTA',   'Dificultad alta â€” procesos complejos');
...
('COLOR',    'Variacion por color â€” misma molderia, distinto color'),
('TELA',     'Variacion por tela â€” misma molderia, distinta tela'),
('MOLDERIA', 'Variacion por molderia â€” cambios en el patron');
...
('ACTIVA',     'Nota activa â€” en uso'),
('UTILIZADA',  'Nota utilizada â€” ya aplicada a produccion'),
('ANULADA',    'Nota anulada â€” cancelada');
...
-- ╔══════════════════════════════════════════════════════════════
-- ║  PARTE 3: SEED FASES WORKFLOW
-- ╚══════════════════════════════════════════════════════════════

-- ===========================================================================
-- AtelierData v2.0 â€” Seed de Fases del Workflow (13 sub-fases)
-- Ejecutar DESPUES de database_schema.sql
-- ===========================================================================
SET search_path = jo, public;

INSERT INTO jo.workflow_phases (code, name, description, sequence, macro_fase) VALUES
-- FASE 1: IDEACION Y DISENO (Area Creativa) â€” azul/frio
('1.1', 'Perfilamiento',
...
-- FASE 2: LABORATORIO Y PROTOTIPADO (Modulo Taller) â€”ambar/tibio
('2.1', 'Alistamiento',
...
-- FASE 3: VALIDACION TECNICA (Area de Ingenieria) â€” naranja/caliente
...
-- FASE 4: INDUSTRIALIZACION (Area de Produccion) â€” rojo/muy caliente
